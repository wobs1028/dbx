...
| 6 | `src-tauri/src/commands/update.rs` | 保持原始代码（从上游恢复） | 检查/下载/安装更新逻辑 |
| 7 | `.github/workflows/build-windows.yml`（新增） | CI 编译 + 签名 + 自动创建 Release + 生成 latest.json | CI 自动编译并签名 |

> 注意：文件 2-6 是从**上游原版恢复**，而非禁用。关键在于文件 2 中替换为**自己的公钥**，其余文件保持上游原样即可。

---

## 前置准备：生成签名密钥对

自动更新需要 Tauri 对安装包进行 minisign 签名，客户端用公钥验签。**必须使用 Tauri 官方 CLI 生成密钥**（`minisign` 命令行生成的密钥格式与 Tauri 不完全兼容，会导致应用启动闪退）。

```powershell
# Windows PowerShell
npx @tauri-apps/cli@2 signer generate -w .\key.key
# 输入密码：dbx-internal（两次）
```

生成两个文件：
- `key.key` — 私钥文件
- `key.key.pub` — 公钥文件（屏幕上也会打印公钥）

**公钥**：`key.key.pub` 的内容直接写入 `tauri.conf.json` 的 `updater.pubkey` 字段。

**私钥（用于 GitHub Secret）**：整个 `key.key` 文件内容的 base64 编码：

```powershell
# 在 Windows PowerShell 中执行：
[Convert]::ToBase64String([System.Text.Encoding]::UTF8.GetBytes((Get-Content .\key.key -Raw)))
```

或者在 Linux 上：

```bash
cat key.key | base64 -w0
```

生成的 base64 字符串作为 `TAURI_SIGNING_PRIVATE_KEY` GitHub Secret 的值。
...
在 `plugins` 下添加 `updater` 节点，`createUpdaterArtifacts` 改为 `true`：
...
`pubkey` 填写上一步生成的 Tauri 格式公钥（base64 一行），`endpoints` 指向内网 nginx 上的 `latest.json`。
...
| `src-tauri/src/commands/update.rs` | 保持上游原始代码（不需要改） |
...
到仓库 Settings → Secrets and variables → Actions → Repository secrets，添加两个 secret：

| Name | Value |
|---|---|
| `TAURI_SIGNING_PRIVATE_KEY` | 私钥 base64 行（**只有第二行**，不要注释行） |
| `TAURI_SIGNING_PRIVATE_KEY_PASSWORD` | 生成密钥时设置的密码（如 `dbx-internal`） |
...
```cmd
set TAURI_SIGNING_PRIVATE_KEY=RWRTY0IygS...（私钥 base64 行）
set TAURI_SIGNING_PRIVATE_KEY_PASSWORD=dbx-internal
...
```powershell
$env:TAURI_SIGNING_PRIVATE_KEY='RWRTY0IygS...（私钥 base64 行）'
$env:TAURI_SIGNING_PRIVATE_KEY_PASSWORD='dbx-internal'
pnpm tauri build -- --no-default-features --features duckdb-bundled,mq-admin
```

> 不设这两个环境变量会报 `failed to decode secret key` 错误。
